...
export default function ExpenseIncomeBreakdown() {
  ...
  const [visible, setVisible] = useState(['income', 'expense', 'net']);
  ...
  const handleToggle = (_, newVisible) => {
  ...
  const toggles = (
    ...
    >
      <ToggleButton value="income" sx={{ color: '#10b981' }}>Income</ToggleButton>
      <ToggleButton value="expense" sx={{ color: '#ef4444' }}>Expense</ToggleButton>
      <ToggleButton value="net" sx={{ color: '#6366f1' }}>Net</ToggleButton>
    </ToggleButtonGroup>
  );
    ...
    >
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={dataWithNet} margin={{ top: 20, right: 20, bottom: 5, left: 10 }}>
          ...
          <Tooltip formatter={(value) => formatCurrency(value)} labelFormatter={formatLabel} />
          ...
          {visible.includes('income') && (
            <Bar dataKey="income" name="Income" fill="url(#incomeGrad)" radius={[6, 6, 0, 0]}>
              {showLabels && <LabelList dataKey="income" position="top" fontSize={11} fontWeight={600} fill={chart.labelFill} formatter={(v) => abbreviateCurrency(v)} />}
            </Bar>
          )}
          {visible.includes('expense') && (
            <Bar dataKey="expense" name="Expenses" fill="url(#expenseGrad)" radius={[6, 6, 0, 0]}>
              {showLabels && <LabelList dataKey="expense" position="top" fontSize={11} fontWeight={600} fill={chart.labelFill} formatter={(v) => abbreviateCurrency(v)} />}
            </Bar>
          )}
          {visible.includes('net') && (
            <Bar dataKey="net" name="Net" fill="url(#netGrad)" radius={[6, 6, 0, 0]}>
              {showLabels && <LabelList dataKey="net" position="top" fontSize={11} fontWeight={600} fill={chart.labelFill} formatter={(v) => abbreviateCurrency(v)} />}
            </Bar>
          )}
        </BarChart>
      </ResponsiveContainer>
