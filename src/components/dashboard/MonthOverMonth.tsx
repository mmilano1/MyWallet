    ...
    <ChartCard title="Month-over-Month" subtitle="Spending % change vs prior month" height={300}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={monthOverMonth} margin={{ top: 20, right: 20, bottom: 5, left: 10 }}>
          ...
          <Tooltip formatter={((value: number) => `${value.toFixed(1)}%`) as never} labelFormatter={(l: unknown) => String(l)} />
          <ReferenceLine y={0} stroke="#94a3b8" strokeDasharray="4 4" />
          <Bar dataKey="changePercent" name="Change %" radius={[6, 6, 6, 6]} activeBar={false}>
            ...
            <LabelList dataKey="changePercent" position="top" fontSize={11} fontWeight={600} fill="#334155" formatter={(v: unknown) => `${Number(v).toFixed(0)}%`} />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </ChartCard>
