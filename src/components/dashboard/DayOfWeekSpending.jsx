    ...
    >
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={dayOfWeekSpending} margin={{ top: 20, right: 20, bottom: 5, left: 10 }}>
          ...
          <Tooltip
            formatter={(value) => [formatCurrency(value), metric === 'avg' ? 'Average' : 'Total']}
          />
          <Bar dataKey={metric} name={metric === 'avg' ? 'Average' : 'Total'} radius={[6, 6, 0, 0]}>
            {dayOfWeekSpending.map((_, i) => (
              <Cell key={i} fill={`url(#dayGrad${i})`} />
            ))}
            <LabelList dataKey={metric} position="top" fontSize={11} fontWeight={600} fill={chart.labelFill} formatter={(v) => abbreviateCurrency(v)} />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </ChartCard>
