...
const ExpenseAmountCell = styled(TableCell)({
  color: '#ef4444',
  fontWeight: 500,
});
          ...
          >
            ...
            >
              ...
              <LabelList
                ...
                formatter={(v: unknown) => abbreviateCurrency(Number(v))}
              />
            </Bar>
          </BarChart>
        </ResponsiveContainer>
          ...
          <>
            ...
            <StyledTableContainer>
              <Table size="small" stickyHeader>
                <TableHead>
                  <TableRow>
                    <BoldHeaderCell>Date</BoldHeaderCell>
                    <BoldHeaderCell>Payee</BoldHeaderCell>
                    <BoldHeaderCell align="right">Amount</BoldHeaderCell>
                    <BoldHeaderCell>Note</BoldHeaderCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {categoryTransactions.map((t, i) => (
                    <TableRow key={i} hover>
                      <DateCell>{formatDate(t.date)}</DateCell>
                      <TableCell>{t.payee || '-'}</TableCell>
                      <ExpenseAmountCell align="right">
                        {formatCurrency(Math.abs(t.amount))}
                      </ExpenseAmountCell>
                      <NoteCell>{t.note || '-'}</NoteCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
