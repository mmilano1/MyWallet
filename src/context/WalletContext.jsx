...
function getLatestYear(transactions) {
...
export function WalletProvider({ children }) {
  ...
  const setPeriodMode = useCallback((m) => dispatch({ type: 'SET_PERIOD_MODE', payload: m }), []);
  const setYear = useCallback((y) => dispatch({ type: 'SET_YEAR', payload: y }), []);
  const setMonth = useCallback((m) => dispatch({ type: 'SET_MONTH', payload: m }), []);
  const setCategoryFilter = useCallback(
    (c) => dispatch({ type: 'SET_CATEGORY_FILTER', payload: c }),
    []
  );
  const setAccountFilter = useCallback(
    (a) => dispatch({ type: 'SET_ACCOUNT_FILTER', payload: a }),
    []
  );
  const setExcludeCategories = useCallback(
    (c) => dispatch({ type: 'SET_EXCLUDE_CATEGORIES', payload: c }),
    []
  );
