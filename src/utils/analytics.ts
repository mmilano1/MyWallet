...
export function groupByPeriod(transactions: Transaction[], periodMode: 'monthly' | 'yearly'): PeriodData[] {
  const groups: Record<string, PeriodData> = {};
...
export function computeSpendingTrends(transactions: Transaction[]): SpendingTrendPoint[] {
  const expenses = transactions.filter(isExpense);
  const groups: Record<string, SpendingTrendPoint> = {};

  for (const t of expenses) {
    const dayKey = format(t.date, 'yyyy-MM-dd');
    if (!groups[dayKey]) {
      groups[dayKey] = { date: dayKey, amount: 0, cumulative: 0 };
    }
...
export function groupByPaymentType(transactions: Transaction[]): PaymentTypeData[] {
  const groups: Record<string, PaymentTypeData> = {};
...
export function computeTopPayees(transactions: Transaction[], limit: number = 10): TopPayee[] {
  const expenses = transactions.filter(isExpense);
  const groups: Record<string, TopPayee> = {};
...
export function computeAccountSummary(transactions: Transaction[]): AccountSummaryRow[] {
  const groups: Record<string, { account: string; income: number; expense: number; count: number }> = {};
...
export function getAvailableMonths(transactions: Transaction[], year: number | null): string[] {
  if (!year) return [];
  const months = [
...
export function getAvailableAccounts(transactions: Transaction[]): string[] {
  return [...new Set(transactions.map((t) => t.account))].sort();
}

export function computeMonthlyAvgDailySpend(transactions: Transaction[]): MonthlyAvgDaily[] {
  const expenses = transactions.filter(isExpense);
  const groups: Record<string, { monthKey: string; total: number; date: Date }> = {};
...
export function computeDayOfWeekSpending(transactions: Transaction[]): DayOfWeekData[] {
  const expenses = transactions.filter(isExpense);
...
export function computeSavingsRate(transactions: Transaction[]): SavingsRateData[] {
  const groups: Record<string, { monthKey: string; income: number; expense: number }> = {};
...
export function computeSpendingByHour(transactions: Transaction[]): HourlySpendingData[] {
  const expenses = transactions.filter(isExpense);
...
export function computeAvgSpendByCategory(transactions: Transaction[], limit: number = 12): AvgSpendByCategoryData[] {
  const expenses = transactions.filter(isExpense);
...
export function computeMonthOverMonth(transactions: Transaction[]): MonthOverMonthData[] {
  const expenses = transactions.filter(isExpense);
